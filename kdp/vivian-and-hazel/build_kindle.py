#!/usr/bin/env python3
"""Build Kindle files for the two Vivian & Hazel stories.

Produces, for each title:
  *-kindle.docx     ← upload this to KDP first
  *-kindle.epub     ← EPUB 2 fallback
  kindle-cover.jpg
...
ABOUT_AUTHOR = (
    ...
    "courage, secrets, and the quiet moments that change everything."
)

BOOKS = [
    {
        "id": "shy-girl",
        "slug": "the-shy-girl-and-the-popular-girl",
        "html": SITE / "shy-girl.html",
        "cover_src": SITE / "images/shy-girl/cover.png",
        "title": "The Shy Girl & The Popular Girl",
        "title_lines": ["THE SHY GIRL", "& THE POPULAR GIRL"],
        "subtitle": "A Vivian & Hazel story",
        "blurb": (
            "Vivian is excited to start school again — and even more thrilled "
            "to help and make friends with new girl Hazel. But do Vivian’s "
            "friends approve of Hazel, or even let her be Hazel’s friend?"
        ),
        "dedication": "A story about choosing kindness — even when it isn’t the easiest thing to do.",
        "end_line": "Two girls. One choice. A friendship that felt right.",
        "uuid_url": "https://anshikamahesh.com/stories/shy-girl.html",
        "next_note": "The next Vivian & Hazel story is A Play of Their Own.",
        "overlay": (47, 111, 126, 168),
        "expected_chapters": 8,
    },
    {
        "id": "play",
        "slug": "a-play-of-their-own",
        "html": SITE / "vivian-and-hazel.html",
        "cover_src": SITE / "images/vivian-and-hazel/cover.png",
        "title": "A Play of Their Own",
        "title_lines": ["A PLAY OF", "THEIR OWN"],
        "subtitle": "A Vivian & Hazel story",
        "blurb": (
            "Hazel isn’t the shy new girl any more. Then Noele arrives from her "
            "old school — and Bella still wants to break the friendship that lasted."
        ),
        "dedication": "Best after The Shy Girl & The Popular Girl.",
        "end_line": "Hurrah for Vivian and Hazel!",
        "uuid_url": "https://anshikamahesh.com/stories/vivian-and-hazel.html",
        "next_note": "",
        "overlay": (122, 78, 138, 168),
        "expected_chapters": 7,
    },
]
...
def parse_blocks(body: str, story_html: Path) -> list[dict]:
    ...
    blocks: list[dict] = []
    for gallery, figure, script, playbox, chant, para in token.findall(body):
        if gallery:
...
def build_cover_jpeg(book: dict, out_path: Path) -> None:
    ...
    try:
        title_font = ImageFont.truetype(str(FONT_DIR / "Georgia Bold.ttf"), 78)
        sub_font = ImageFont.truetype(str(FONT_DIR / "Georgia Italic.ttf"), 34)
...
def toc_ncx(book: dict, data: dict) -> str:
    ...
    if data["cast"]:
        points.append(
            f'<navPoint id="nav-cast" playOrder="{order}"><navLabel><text>Best friends, still</text></navLabel><content src="cast.xhtml"/></navPoint>'
        )
        order += 1
    for ch in data["chapters"]:
        n = f"{ch['num']:02d}"
...
def nav_xhtml(data: dict) -> str:
    ...
    if data["cast"]:
        items.append('<li><a href="cast.xhtml">Best friends, still</a></li>')
    for ch in data["chapters"]:
        items.append(
...
def build_epub(book: dict, data: dict, cover_path: Path, epub_path: Path) -> int:
    ...
    if data["cast"]:
        ...
        add("OEBPS/cast.xhtml", xhtml("Best friends, still", "<h1>Best friends, still</h1>\n" + rows))
    toc_items = []
    if data["cast"]:
        toc_items.append('<p class="first"><a href="cast.xhtml">Best friends, still</a></p>')
    for ch in data["chapters"]:
        toc_items.append(
...
def build_docx(book: dict, data: dict, docx_path: Path) -> None:
    ...
    if data["cast"]:
        toc_entries.append(("cast", "Best friends, still"))
    for ch in data["chapters"]:
    ...
    if data["cast"]:
        h = doc.add_heading("Best friends, still", level=1)
...
<p><b>Vivian is popular. Hazel is new, and shy. Can they be friends?</b></p>
<p>Vivian is excited to start school again — and even more thrilled to help and make friends with new girl Hazel. But do Vivian’s friends approve of Hazel, or even let her be Hazel’s friend? There’s drama for sure: badminton tryouts, a small birthday party, and a choice that isn’t the easiest thing to do.</p>
<p>So begins <i>The Shy Girl &amp; The Popular Girl</i>, a school story by young author Anshika Mahesh. Told in both girls’ voices, it is about kindness, courage, and what true friendship looks like.</p>
<p><b>This book is for:</b></p>
<ul>
<li>Readers ages 9–12 who like school stories</li>
<li>Families who want a short illustrated chapter book about friendship</li>
<li>Anyone who has ever had to choose between fitting in and being kind</li>
</ul>
<p>Eight chapters, with pictures. A <i>Vivian &amp; Hazel</i> story.</p>
<p><i>The Shy Girl &amp; The Popular Girl</i> · Written by Anshika Mahesh</p>
"""
...
1. school friendship story for girls 9 to 12
2. shy new girl finds a true friend
3. middle grade school kindness chapter book
4. badminton team tryouts girls fiction
5. choosing friends over popularity kids
6. dual point of view school story
7. Indian school story middle grade
"""
    else:
        desc = """Paste into KDP → Kindle eBook → Description. Amazon shows the first paragraph before “Read more,” so keep that hook as-is.
...
<p><b>Hazel isn’t the shy new girl any more. Then an old rival arrives — and the class gets to write the school play.</b></p>
<p>Hazel and Vivian are best friends now. Then Noele arrives from Hazel’s old school, Bella still wants to break them apart, and Miss Colman lets the girls write a play of their own. There are rumours, a secret about Vivian’s sister, jokes, costumes, and opening night.</p>
<p><i>A Play of Their Own</i> is a school story by young author Anshika Mahesh. Best after <i>The Shy Girl &amp; The Popular Girl</i>.</p>
<p><b>This book is for:</b></p>
<ul>
<li>Readers ages 9–12 who liked the first Vivian &amp; Hazel story</li>
<li>Families who enjoy school plays, friendship, and a bit of drama</li>
<li>Anyone who wants a story about standing up for a friend</li>
</ul>
<p>Seven chapters, with pictures and costume drawings. A <i>Vivian &amp; Hazel</i> story.</p>
<p><i>A Play of Their Own</i> · Written by Anshika Mahesh</p>
"""
        keys = """Paste one phrase into each of KDP’s 7 keyword boxes. Do not repeat the title or author name. Each is under 50 characters.

1. school play chapter book for girls
2. best friends write a class play
3. rumours and rivals middle grade
4. circus clown school play story
5. standing up for a friend kids book
6. middle grade school drama kindness
7. sister comes home friendship story
"""
...
def build_one(book: dict) -> None:
    out_dir = HERE / ("shy-girl" if book["id"] == "shy-girl" else "a-play-of-their-own")
